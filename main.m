...
dist_raw = sqrt(train_x(:,ind(1:100))' * train_x(:,ind(1:100)));
dist_raw = dist_raw ./ max(dist_raw(:));

dist_var = sqrt(train_y(:,ind(1:100))' * train_y(:,ind(1:100)));
dist_var = dist_var ./ max(dist_var(:));

sup_z1 = [train_x;ones(1,size(train_x,2))]' * supNet.wIn;
sup_rep  = func(sup_z1,p_func);
dist_sup_rep = sqrt(sup_rep(ind(1:100),:) * sup_rep(ind(1:100),:)');
dist_sup_rep = dist_sup_rep ./ max(dist_sup_rep(:));

uns_z1 = [train_x;ones(1,size(train_x,2))]' * unsNet.wIn;
uns_rep  = func(uns_z1,p_func);
dist_uns_rep = sqrt(uns_rep(ind(1:100),:) * uns_rep(ind(1:100),:)');
dist_uns_rep = dist_uns_rep ./ max(dist_uns_rep(:));

figure('name','distances raw');pcolor(dist_raw);
figure('name','distances var');pcolor(dist_var);
figure('name','distances sup');pcolor(dist_sup_rep);
figure('name','distances uns');pcolor(dist_uns_rep);